...
def remember_contact(client):
    """Отметить, что человек нам писал: имя и короткое имя его аккаунта.

    Без этой отметки не работает связка аккаунтов: когда человек пришлёт
    «@d_chul» или «vk.com/chuul», найти по такому имени номер можно только
    у себя — мессенджеры чужой номер по имени не отдают. Отсюда же берётся
    ответ «сообщение от такого пользователя не найдено».

    Спрашиваем мессенджер не на каждом сообщении, а раз в сутки: в ВК это
    сетевой запрос, а имя и короткое имя меняются от силы раз в год. Пустое
    короткое имя — повод спросить снова: значит, в прошлый раз узнать
    не удалось (ВК не ответил) либо человек его только что завёл.
    """
    known = db.get_contact(client.platform, client.id)
    if (known and known["handle"]
            and known["seen_date"] == date.today().isoformat()):
        return

    name, handle = bot.contact(client)
    db.save_contact(client.platform, client.id, name, handle)
...
if __name__ == "__main__":
    ...
    for message in bot.listen():
        ...
        # До замка: в ВК это сетевой запрос, и держать из-за него замок,
        # пока фоновый поток ждёт своей очереди, незачем. Своя try/except —
        # не сумели запомнить контакт, так это мешает только связке аккаунтов,
        # а ответить клиенту бот обязан в любом случае.
        try:
            remember_contact(message.client)
        except Exception as error:
            print(f"Не смогла запомнить контакт: {error}")
