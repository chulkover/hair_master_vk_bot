...
def save_booking(user_id):
    """Сохранить запись в файл — если время свободно и лимит не исчерпан."""
    user = get_user(user_id)

    # Ещё одна проверка лимита, прямо перед записью в файл. Та, в ask_date(),
...
#
# Что «напоминание уже отправлено», помнит не поток, а статус записи в файле:
...
# которым напомнили ДО перезапуска: в файле времени напоминания нет, а
...
def run_scheduler():
    ...
    while True:
        ...
        # к longpoll, и лезть в файл в ту же секунду незачем.
